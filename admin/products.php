...
try {
    ...
    if (isset($_FILES['csv_file']) && $_FILES['csv_file']['error'] === UPLOAD_ERR_OK) {
        ...
        if ($file_ext !== 'csv') {
            $error_message = "Veuillez télécharger un fichier CSV valide.";
            break;
        }
        
        // Open and read CSV file with proper encoding detection
        if (($handle = fopen($file_tmp, 'r')) !== FALSE) {
            
            // Detect and handle BOM (Byte Order Mark) for UTF-8 files
            $bom = fread($handle, 3);
            if ($bom !== "\xEF\xBB\xBF") {
            ...
            if ($header === false || empty($header)) {
                $error_message = "Le fichier CSV est vide ou mal formaté.";
                fclose($handle);
                break;
            }
            
            // Clean headers (trim whitespace and convert to lowercase)
            $header_lower = array_map(function($h) {
                return strtolower(trim($h));
            }, $header);
            
            // Validate required headers
            $required_headers = ['code', 'name', 'price', 'sellingprice'];
            $missing_headers = [];
            
            foreach ($required_headers as $req_header) {
                if (!in_array($req_header, $header_lower)) {
                    $missing_headers[] = $req_header;
                }
            }
            
            if (!empty($missing_headers)) {
                $error_message = "Colonnes manquantes dans le CSV: " . implode(', ', $missing_headers);
                fclose($handle);
                break;
            }
            ...
            while (($data = fgetcsv($handle, 0, ',')) !== FALSE) {
                ...
                try {
                    // Skip rows that don't have the same number of columns as header
                    if (count($data) !== count($header)) {
                        $upload_errors[] = "Ligne $row_number: Nombre de colonnes incorrect (attendu: " . count($header) . ", trouvé: " . count($data) . ")";
                        continue;
                    }
                    ...
                    foreach ($header_lower as $index => $key) {
                        $row_data[$key] = isset($data[$index]) ? trim($data[$index]) : '';
                    }
                    
                    // Extract and validate required fields
                    $code = $row_data['code'] ?? '';
                    $name = $row_data['name'] ?? '';
                    $price = floatval($row_data['price'] ?? 0);
                    $sellingPrice = floatval($row_data['sellingprice'] ?? 0);
                    
                    // Validate required fields
                    if (empty($code)) {
                        $upload_errors[] = "Ligne $row_number: Code produit manquant";
                        continue;
                    }
                    
                    if (empty($name)) {
                        $upload_errors[] = "Ligne $row_number: Nom produit manquant";
                        continue;
                    }
                    
                    if ($price <= 0) {
                        $upload_errors[] = "Ligne $row_number: Prix invalide (doit être > 0)";
                        continue;
                    }
                    
                    if ($sellingPrice <= 0) {
                        $upload_errors[] = "Ligne $row_number: Prix de vente invalide (doit être > 0)";
                        continue;
                    }
                    ...
                    $stock = intval($row_data['stock'] ?? 0);
                    $purchasePrice = floatval($row_data['purchaseprice'] ?? $price);
                    $vatRate = floatval($row_data['vatrate'] ?? 18);
                    
                    // Handle expiry date
                    $expiryDate = null;
                    if (!empty($row_data['expirydate'] ?? '')) {
                        $dateStr = trim($row_data['expirydate']);
                    ...
                    $statut_TVA = trim($row_data['statut_tva'] ?? 'taxable');
                    ...
                    if ($result) {
                        ...
                        $upload_errors[] = "Ligne $row_number: Erreur lors de l'insertion en base de données";
                    }
                    
                } catch (Exception $e) {
                    $upload_errors[] = "Ligne $row_number: " . $e->getMessage();
                }
            }
            ...
            $messages = [];
            if ($upload_success > 0) {
                $messages[] = "$upload_success produit(s) importé(s) avec succès";
            }
            if ($upload_skipped > 0) {
                $messages[] = "$upload_skipped produit(s) ignoré(s) (codes existants)";
            }
            if (!empty($upload_errors)) {
                $messages[] = count($upload_errors) . " erreur(s) détectée(s)";
            }
            
            if ($upload_success > 0) {
                $success_message = implode('. ', $messages);
            } else {
                $error_message = "Aucun produit n'a été importé. " . implode('. ', $messages);
            }
        ...
        $error_message = $upload_error_codes[$upload_error] ?? "Erreur lors du téléchargement du fichier.";
    }
    break;
            }
        }
...
<head>
    ...
    <title>digiPharm - Gestion des Produits</title>
...
<div class="export-buttons">
    <button onclick="openExportModal()" class="btn-export" style="background: var(--ds-green);">
    ...
    <button onclick="printReport()" class="btn-export">
        ...
        Imprimer
                ...
                <div class="products-section">
                 <!-- Find this section around line 570 and add the upload button -->
<div class="section-header">
    ...
    <div style="display: flex; gap: 0.5rem;">
        <a href="print-barcodes.php<?php echo (!empty($search) || $categoryFilter > 0 || $supplierFilter > 0) ? '?search=' . urlencode($search) . '&category=' . $categoryFilter . '&supplier=' . $supplierFilter : ''; ?>" 
        ...
        <button class="btn-add-product" data-bs-toggle="modal" data-bs-target="#uploadCsvModal" style="background: var(--ds-green);">
...
<div class="modal fade" id="exportExcelModal" tabindex="-1">
                ...
                <div class="modal-body">
                    <div class="mb-3">
                        <label for="exportCategory" class="form-label">
                            <strong>Sélectionner une catégorie</strong>
                        </label>
                        <select class="form-control" id="exportCategory" name="category">
                        ...
                        <small class="text-muted mt-2 d-block">
                            ...
                            Laissez vide pour exporter tous les produits
                        </small>
                    </div>
                    ...
                    <div class="alert alert-info" style="font-size: 0.875rem;">
                        ...
                        <ul class="mb-0 mt-2">
                            <li>Informations complètes des produits</li>
                            <li>Statistiques du stock</li>
                            <li>Mise en forme Excel professionnelle</li>
                            <li>Dates et valeurs formatées</li>
                        </ul>
                    </div>
                </div>
    ...
    <label for="productCode" class="form-label">
        Code Produit (Scanner ou Taper) *
        <i data-lucide="scan-barcode" style="width: 16px; height: 16px; margin-left: 5px;"></i>
    </label>
    <div style="position: relative;">
        ...
        <div style="position: absolute; right: 10px; top: 50%; transform: translateY(-50%); color: #3b82f6;">
            <i data-lucide="scan-line"></i>
        </div>
    </div>
    <small class="text-muted">
        <i data-lucide="info" style="width: 12px; height: 12px;"></i>
        Utilisez votre lecteur pour scanner le code-barres directement dans ce champ
    </small>
</div>
                        </div>
...
<div class="modal fade" id="uploadCsvModal" tabindex="-1">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <form method="POST" enctype="multipart/form-data" id="uploadCsvForm">
                <div class="modal-body">
                    ...
                    <div class="alert alert-info">
                        ...
                        <ul class="mb-0 mt-2">
                            ...
                            <li>Colonnes <strong>obligatoires</strong>: code, name, price, sellingPrice</li>
                            <li>Colonnes <strong>optionnelles</strong>: description, stock, purchasePrice, vatRate, expiryDate, category, supplier, statut_tva</li>
                            <li>Les produits avec des codes existants seront ignorés</li>
                            <li>Format date d'expiration: YYYY-MM-DD</li>
                            <li><strong class="text-success">Les catégories et fournisseurs inconnus seront créés automatiquement</strong></li>
                        </ul>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="file" 
                               ...
                               accept=".csv"
                               required>
                        <small class="text-muted">Taille maximale: 5 MB</small>
                    </div>

                    <div class="card">
                        <div class="card-header bg-light">
                            <strong>Exemple de fichier CSV:</strong>
                        </div>
                        <div class="card-body">
                            <pre class="mb-0" style="font-size: 0.8rem; overflow-x: auto;">code,name,description,price,purchasePrice,sellingPrice,stock,vatRate,expiryDate,category,supplier,statut_tva
PROD001,Paracétamol 500mg,Antalgique,500,400,600,100,18,2025-12-31,Médicaments,Pharma Suppliers,taxable
PROD002,Vitamine C,Complément alimentaire,1000,800,1200,50,18,2026-06-30,Vitamines,Health Corp,taxable</pre>
                        </div>
                    </div>
                    ...
                    <div class="alert alert-warning mt-3">
                        <strong>Erreurs détectées lors de l'importation:</strong>
    ...
    <script>
        ...
        function printReport() {
            ...
            printWindow.document.write(`
                ...
                <html>
                <head>
                    <title>Rapport des Produits - digiPharm</title>
                    <style>
    ...
    const csvContent = `code,name,description,price,purchasePrice,sellingPrice,stock,vatRate,expiryDate,category,supplier,statut_tva
PROD001,Paracétamol 500mg,Antalgique,500,400,600,100,18,2025-12-31,Médicaments,Pharma Suppliers,taxable
PROD002,Vitamine C,Complément alimentaire,1000,800,1200,50,18,2026-06-30,Vitamines,Health Corp,taxable`;
...
// Add keyboard shortcut Ctrl+U for upload
document.addEventListener('keydown', function(e) {
    if ((e.ctrlKey || e.metaKey) && e.key === 'u') {
        e.preventDefault();
        const uploadModal = new bootstrap.Modal(document.getElementById('uploadCsvModal'));
        uploadModal.show();
    }
});
