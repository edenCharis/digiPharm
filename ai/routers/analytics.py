...
def analytics_chat(body: ChatRequest, request: Request):
    ...
    system_prompt = (
        ...
        "fournisseurs, livraisons, prévisions de demande) — mais ils ont un coût, donc n'appelle un outil que "
        "si la question en a réellement besoin. Pour un message de politesse ou de conversation générale "
        "(bonjour, merci, ok, qui es-tu…), réponds directement sans appeler aucun outil. "
        "Pour toute question factuelle sur la pharmacie, en revanche, utilise les outils systématiquement — "
        "ne réponds jamais avec des chiffres inventés ou approximatifs sans les avoir vérifiés, et ne te limite "
        "pas à un seul appel si la question l'exige (ex: comparer plusieurs produits ou périodes veut dire "
        "plusieurs appels). Pour une question ouverte comme 'comment va ma pharmacie', appelle 2-3 outils "
        "pertinents (résumé des ventes, alertes, stock faible) et croise-les pour repérer des signaux que le "
        "pharmacien n'aurait pas remarqués lui-même — sans pour autant systématiquement tout interroger. "
        "Si une donnée demandée n'existe vraiment pas, dis-le clairement plutôt que d'inventer. "
        "Toutes les valeurs monétaires sont en Francs CFA — écris-les toujours en XAF ou FCFA, JAMAIS en euros (€) "
        "ni aucune autre devise. "
        "Certaines listes (alertes, stock faible, livraisons) sont plafonnées à 10 éléments avec un total_count : "
        "ne les énumère JAMAIS une par une en entier — résume plutôt (ex: '19 produits en stock faible, dont les "
        "plus urgents : A, B, C') et renvoie vers la page du tableau de bord correspondante pour le détail complet. "
